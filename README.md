...
* v0.2.0 以降では、オリジナルコンパイラでのBASIC文の以下の書式も受け付けます。

  ```text
  1000 "D   t8,3L48L+32 m12,1,2,1 J
  1010 "D   e24f48f+16&f+48f32e2.&e32&e24
  ```
...
* コンパイル途中でエラーが発生した場合でも、MMLファイルのコンパイルは
  ...
  正しく行われません。
* MML 記述中にタブが含まれる場合、コンパイラとしてはタブを読み飛ばしますが、
  タブ以降でエラーがあった場合のエラー位置表示についてはタブ分の調整は行いません
  (タブの分だけエラー発生位置表示はズレて表示されるのでタブの使用は非推奨です)。
* オリジナルのドライバのマニュアルで「ネストに関する謎」という説明にあるとおり
  ...
  * オリジナルコンパイラの仕様はループ先頭にオクターブ指定がある場合も
    「ループ前の最後の発声オクターブから変化がある場合のみオクターブコマンド出力」
    という動作なので、ループ2回目のオクターブが正しく設定されないケースがあります。
    v0.2.0 以降で「ループ内最初のオクターブは必ず出力する」ように仕様を変更しています。
...
- v0.2.1 (2025/12/17)
  - コンパイルエラー表示関連修正
  - バグ修正: 行番号付き書式や先頭にスペースがある場合にエラー表示位置がズレる
    [#5](https://github.com/tsutsui/p6psgmmlc/issues/5)
  - 仕様修正: 最終行ネストエラーのエラー位置表示について ';' があるときはその位置にする

- v0.2.0 (2025/12/12)
  - MML2P6PSGDRV 付属サンプル `SAMPLEMML.txt` の演奏不具合修正
  - 仕様追加: `行番号 "[パート]` のオリジナルコンパイラBASIC書式も受け付け可能にする
    [#1](https://github.com/tsutsui/p6psgmmlc/issues/1)
  - バグ修正: 負のパラメータについて符号と数値を独立に書き込むパラメータを修正
    [#2](https://github.com/tsutsui/p6psgmmlc/issues/2)
  - 仕様修正: `[` `]` のネスト最初のオクターブがリピート時にズレることがある問題対応
    [#3](https://github.com/tsutsui/p6psgmmlc/issues/3)
